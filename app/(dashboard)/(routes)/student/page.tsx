...
const StudentPage = async () => {
  ...
  try {
    ...
    // Preparar datos para el componente móvil
    const mobileCourses = coursesWithPurchaseInfo.map(course => ({
      id: course.id,
      title: course.title,
      progress: Math.floor(Math.random() * 101), // Placeholder - reemplazar con progreso real
      totalChapters: course.chapters.length,
      completedChapters: Math.floor(course.chapters.length * 0.6), // Placeholder
      instructor: "Instituto San Pablo",
      image: course.imageUrl || "/placeholder-course.jpg",
      category: course.category?.name || "General",
      nextChapter: course.chapters.length > 0 ? "Siguiente capítulo disponible" : undefined,
    ...
    return (
      <>
        {/* Componente móvil - solo visible en dispositivos móviles */}
        <div className="block md:hidden">
          <MobileStudentDashboard courses={mobileCourses} />
        </div>

        {/* Diseño de PC - solo visible en desktop */}
        <div className="hidden md:block relative min-h-screen bg-black text-white font-sans antialiased">
